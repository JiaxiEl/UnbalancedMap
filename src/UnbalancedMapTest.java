...
public class UnbalancedMapTest {
    ...
    private static List<Long> getBalancedKeySequence(long start,
                                                     long stop) {

        List<Long> toReturn = new LinkedList<>();

        if (start > stop) return toReturn;
    ...
    private static IMap<String, Long> addInOrderSequence(IMap<String,
            Long> target, int testSize, int startIntKey) {

        for (long curKeyNum = startIntKey; curKeyNum < testSize + startIntKey;
             curKeyNum++) {
    ...
    private void checkInitializedCorrectly() {
        ...
        assertTrue(sut.isEmpty(),"Expected: isEmpty() true");
        assertEquals(0, sut.size(),String.format("Expected: size() == 0. Found [%d]", sut.size()));
    ...
    public void getKey_notPresent_returnsNull() {
        ...
        addInOrderSequence(sut, DEFAULT_TEST_SIZE, DEFAULT_START_KEY);
    ...
    public void getKey_returnsCorrectKey() {
        ...
        addInOrderSequence(sut, testSize, DEFAULT_START_KEY);
    ...
    public void keysetAndValues_orderMatches() {
        ...
        for (Long key : getBalancedKeySequence(DEFAULT_START_KEY,
                DEFAULT_TEST_SIZE)) {
        ...
        try {
            ...
            fail("Key and Value iterators should have identical number of " +
                    "entries.");
        }
    }
    ...
    public void mapConstructor_initCorrectly() {
        IMap<String, Long> original =
                addInOrderSequence(new UnbalancedMap<>(),
                        DEFAULT_TEST_SIZE, 0);
    ...
    public void add_duplicateKeys_unchanged() {
        ...
        for (int testKey = DEFAULT_START_KEY; testKey < DEFAULT_START_KEY +
                DEFAULT_TEST_SIZE; testKey++) {
    ...
    @Test
    public void delete_twoChildRoot_correctSizeAndContents() {
        IMap<Integer, Integer> underTest = new UnbalancedMap<>();

        // build the test data
        underTest.add(10, DEFAULT_CHECK);
        underTest.add(5, DEFAULT_IGNORE);
        underTest.add(15, DEFAULT_IGNORE);
        underTest.add(2, DEFAULT_IGNORE);
        underTest.add(7, DEFAULT_IGNORE);
        underTest.add(12, DEFAULT_IGNORE);
        underTest.add(17, DEFAULT_IGNORE);

        // perform the action
        int oldValue = underTest.delete(10);

        // verify the structure
        assertEquals(6, underTest.size());
        assertEquals(DEFAULT_CHECK, oldValue);
    ...
    public void delete_oneChildRoot_correctSizeAndContents() {
        ...
        // build the test data
        underTest.add(1, DEFAULT_CHECK);
        underTest.add(10, DEFAULT_IGNORE);
        underTest.add(5, DEFAULT_IGNORE);
        underTest.add(15, DEFAULT_IGNORE);

        // perform the action
        int oldValue = underTest.delete(1);
    ...
    @Test
    public void delete_oneChildSubchild_correctSizeAndContents() {
        IMap<Integer, Integer> underTest = new UnbalancedMap<>();
        ...
        underTest.add(15, DEFAULT_IGNORE);
        underTest.add(2, DEFAULT_IGNORE);
        underTest.add(1, DEFAULT_IGNORE);
    ...
    public void delete_oddKeys_contentsCorrect() {
        ...
        // build the test data
        addInOrderSequence(sut, testSize, 0);
        ...
        for (int keyCounter = 0; keyCounter < testSize; keyCounter += 2) {
            ...
            assertEquals(keyCounter, (long) sut.getValue(getFormattedKey
                    (keyCounter)));
    ...
    public void contains_identicalValuesDifferentObjects_found() {
        ...
        // build the data
        IMap<TestObject, Integer> underTest = new UnbalancedMap<>();
        ...
        // check the results
        assertFalse(results.contains(false),"Failed to find a valid Comparable equal. Make " +
                "certain the data structure uses the .compareTo method and " +
                "NOT .equals() when checking keys");
    }
